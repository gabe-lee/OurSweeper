...
func FromIndex(index int, yShift int, xMask int) Coord {
	return Coord{
		X: index & xMask,
		Y: index >> yShift,
	}
...
func (b *ByteCoord) ReadWire(r io.Reader, order binary.ByteOrder) error {
	var e utils.ErrorChecker
	if e.IsErr(binary.Read(r, order, b.X)) {
		return e.Err
	}
	if e.IsErr(binary.Read(r, order, b.Y)) {
		return e.Err
	}
	return nil
}
...
func (b *ByteCoord) WriteWire(w io.Writer, order binary.ByteOrder, mode uint32) error {
	var e utils.ErrorChecker
	if e.IsErr(binary.Write(w, order, b.X)) {
		return e.Err
	}
	if e.IsErr(binary.Write(w, order, b.Y)) {
		return e.Err
	}
	return nil
}
...
var _ wire_serializer.WireSerializer = (*ByteCoord)(nil)

var NEAR_OFFSET = [8]Coord{
