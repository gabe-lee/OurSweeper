...
func NewSweepDB(logWriter io.Writer) SweepDB {
	return SweepDB{
		Log: logger.NewLogger("DATABASE", logWriter),
	}
}
...
func (s *SweepDB) CreateNewWorld(world *ServerWorld, difficulty byte) {
	...
	result := attempt_group.NewWithTimeout("create chunks for new world", time.Second*10, int64(WORLD_CHUNK_COUNT))
...
func (s *SweepDB) UpdateAllChunks(world *ServerWorld) {
	result := attempt_group.NewWithTimeout("UpdateAllChunks", time.Second*10, int64(WORLD_CHUNK_COUNT))
	for idx := range WORLD_CHUNK_COUNT {
...
func (s *SweepDB) LoadAllChunks(world *ServerWorld) {
	...
	for rows.Next() {
		...
		cPos := coord.CoordFromIndex(idx, CY_SHIFT, CX_MASK)
		tTopLeft := cPos.ShiftUpScalar(TILE_TO_LOCK_SHIFT)
		bIdx := 0
		for y := range WORLD_TILES_PER_CHUNK_AXIS {
			for x := range WORLD_TILES_PER_CHUNK_AXIS {
				tPos := tTopLeft.AddXY(x, y)
				tIdx := tPos.ToIndex(TY_SHIFT)
	...
	if cnt != WORLD_CHUNK_COUNT {
		s.Log.Warn("only loaded %d/%d chunks from database for world %d", cnt, WORLD_CHUNK_COUNT, id)
	}
}
